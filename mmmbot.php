<?php
$access_token = 'lwUzEX7t6sNpTueq4vTJeI4AkH9z5+rE0E27Rku+3q4ClAc1p217XI198ZIpUjNLGQ8IOLbd7J/+zGgAQhXeXFhEJAoncDE65/uf6W3yTW/evncpXwnEM48fklTfKfsvQm8148zP+NZq/yDvuUFYqAdB04t89/1O/w1cDnyilFU=';
	...
	foreach ($events['events'] as $event) {
		...
		if ($event['type'] == 'message' && $event['message']['type'] == 'text') {
			...
			if($text=="สวัสดี"){
				...
				$textreply=quote_generate($quote);
			}else if(stristr($text,"แม่เจ้า")==TRUE){
				//Random Message
				$quote = array("666","เจ้ามือหรา","ใครๆ","เจ้าแม่");
				$textreply=quote_generate($quote);
			}else if(stristr($text,"กวน")==TRUE){
				//Random Message
				$quote = array("กวนทีนหรา","กวนมะม่วง","ป่าวกวนน๊า","555","วิ่งไปหลบหลังบ้าน");
				$textreply=quote_generate($quote);
			}else if(stristr($text,"รอ")==TRUE){
				//Random Message
				$quote = array("ตาใครอ่ะ","รอไร","รอใคร","รอแล้วรอเล่า เฝ้าแต่รอ","รอต่อไป");
				$textreply=quote_generate($quote);
